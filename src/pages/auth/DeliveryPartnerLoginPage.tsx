...
export function DeliveryPartnerLoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const result = LoginSchema.safeParse({ email_id: emailId, password, role: "delivery_partner" });
    ...
    try {
      ...
      if (loginResult.status === "incomplete") {
        navigate("/signup/delivery-partner");
      } else {
        navigate("/profile");
      }
    } catch (err) {
    ...
    <AuthLayout highlights={highlights}>
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Handle pickups and deliveries across the city
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <Button type="submit" disabled={isLoading}
          className="w-full h-10 font-semibold bg-amber-500 hover:bg-amber-600 border-amber-500">
          {isLoading ? (
            ...
            <span className="flex items-center gap-2"><LogIn className="h-4 w-4" /> Sign in as Delivery Partner</span>
          )}
        </Button>
      </form>
      ...
      <div className="mt-6 text-center text-sm text-slate-500">
        Not registered yet?{" "}
        <Link to="/signup/delivery-partner" className="text-amber-600 font-medium hover:underline dark:text-amber-400">
