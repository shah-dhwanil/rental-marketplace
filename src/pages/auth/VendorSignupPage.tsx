...
import { Eye, EyeOff, Building2, Landmark, CheckCircle, ChevronRight } from "lucide-react";
import { z } from "zod";
...
const STEPS = [
  ...
  { label: "Business", icon: <Building2 className="h-3.5 w-3.5" /> },
...
export function VendorSignupPage() {
  ...
  // Support resuming registration from a specific step (set by login when profile is incomplete)
  ...
  // Clear the resume step from the store after we've read it on mount
  useEffect(() => {
  ...
  const handleStep1 = async (e: FormEvent) => {
    ...
    const result = RegisterStep1Schema.safeParse({ ...s1, role: "vendor" });
  ...
  const handleStep2 = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await authService.vendorStep2(result.data, tempToken);
      setTempToken(res.temp_token);
      setStep(3);
      setFormErrors({});
    } catch (err) {
      setServerError(err instanceof Error ? err.message : "Failed to save business info");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStep3 = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const tokens = await authService.vendorStep3(result.data, tempToken);
    ...
    return (
      <AuthLayout highlights={highlights}>
        <div className="text-center py-6">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">Vendor Account Ready! 🎉</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
            Your vendor profile is under review. Our team will verify your details within 24 hours.
          </p>
    ...
    <AuthLayout highlights={highlights}>
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
          {isResuming ? "Complete Your Registration" : "Join as a Vendor"}
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          {isResuming ? "Pick up where you left off." : (
            <>
              Already have one?{" "}
              <Link to="/login/vendor" className="text-primary font-medium hover:underline">Sign in</Link>
            </>
          )}
        </p>
        {isResuming && (
          <div className="mt-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 px-3 py-2">
            <p className="text-xs text-amber-700 dark:text-amber-300">
              Your account was created. Complete the remaining steps to activate your vendor profile.
            </p>
          </div>
        )}
      </div>
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} noValidate className="space-y-4">
          <Field label="Full Name" id="name" error={formErrors.name}>
            <Input id="name" placeholder="John Doe" value={s1.name}
          ...
          <Field label="Email Address" id="email" error={formErrors.email_id}>
            <Input id="email" type="email" placeholder="you@company.com" value={s1.email_id}
          ...
          <Button type="submit" disabled={isLoading} className="w-full h-10 font-semibold">
            {isLoading ? <Spinner label="Creating account…" /> : "Continue to Business Info →"}
          </Button>
        </form>
      )}
      ...
      {step === 2 && (
        <form onSubmit={handleStep2} noValidate className="space-y-4">
          <Field label="Business Name" id="bname" error={formErrors.name}>
            <Input id="bname" placeholder="Awesome Rentals Pvt. Ltd." value={s2.name}
          ...
          <Field label="Business Address" id="address" error={formErrors.address}>
            <Input id="address" placeholder="123 Main Street, Floor 2" value={s2.address}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="City" id="city" error={formErrors.city}>
              <Input id="city" placeholder="Mumbai" value={s2.city}
            ...
            <Field label="Pincode" id="pin" error={formErrors.pincode}>
              <Input id="pin" placeholder="400001" maxLength={6} value={s2.pincode}
          ...
          <Button type="submit" disabled={isLoading} className="w-full h-10 font-semibold bg-emerald-600 hover:bg-emerald-700">
      ...
      {step === 3 && (
        <form onSubmit={handleStep3} noValidate className="space-y-4">
          <p className="text-xs text-slate-400 -mt-2 mb-1">
            Your bank details are encrypted and used only for rental payouts.
          </p>

          <Field label="Account Holder Name" id="holder" error={formErrors["bank_details.account_holder_name"]}>
            <Input id="holder" placeholder="John Doe" value={s3.bank_details.account_holder_name}
          ...
          <Field label="Bank Name" id="bank" error={formErrors["bank_details.bank_name"]}>
            <Input id="bank" placeholder="HDFC Bank" value={s3.bank_details.bank_name}
          ...
          <Field label="IFSC Code" id="ifsc" error={formErrors["bank_details.ifsc_code"]}>
            <Input id="ifsc" placeholder="HDFC0001234" maxLength={11}
