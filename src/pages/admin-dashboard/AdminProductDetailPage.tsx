...
import { ArrowLeft, X, Cpu, Image, Trash2 } from "lucide-react";
import { z } from "zod";
...
export function AdminProductDetailPage() {
  ...
  const [form, setFormState] = useState<Record<string, string | boolean>>({
    name: "", description: "", category_id: "",
    price_day: "", price_week: "", price_month: "",
    security_deposit: "0", defect_charge: "0", is_active: true,
  });
  ...
  useEffect(() => {
    if (!productId) return;
    Promise.all([
      catalogService.getProduct(productId, accessToken ?? undefined),
      ...
      .then(([p, cats]) => {
        ...
        setProperties(
          typeof p.properties === "object" && p.properties !== null
            ? Object.fromEntries(Object.entries(p.properties).map(([k, v]) => [k, String(v)]))
            : {},
        );
        setFormState({
          name: p.name, description: p.description,
          category_id: p.category_id,
          price_day: String(p.price_day), price_week: String(p.price_week),
  ...
  async function handleDelete() {
    ...
    if (!confirm("Delete this product permanently? All associated devices will also be removed.")) return;
    setDeleting(true);
    try {
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {product.image_urls.length < 8 && (
            <button
              ...
              className="size-24 rounded-lg border-2 border-dashed border-border hover:border-violet-500 flex flex-col items-center justify-center gap-1 text-muted-foreground hover:text-violet-600 transition-colors"
            >
              {uploadingImage ? (
                <div className="size-5 border-2 border-current border-t-transparent rounded-full animate-spin" />
              ) : (
                <><Image className="size-5" /><span className="text-xs">Upload</span></>
              )}
            </button>
          )}
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {(["price_day", "price_week", "price_month"] as const).map((key) => (
              <div key={key} className="space-y-1">
                ...
                <Input type="number" min="0" step="0.01" value={String(form[key])} onChange={(e) => set(key, e.target.value)} />
        ...
        <div className="space-y-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Extra details shown to customers.</p>
          </div>
