...
function SidebarContent({
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Vendor badge */}
      {!collapsed && (
        <div className="mx-3 mt-3 mb-1 rounded-lg bg-primary/10 border border-primary/20 px-3 py-2">
          <p className="text-[10px] font-semibold tracking-widest text-primary uppercase">Vendor Portal</p>
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        {NAV_ITEMS.map((item) => (
          <NavLink
            key={item.href}
            to={item.href}
            end={item.href === "/vendor/dashboard"}
            onClick={onClose}
            className={({ isActive }) =>
              cn(
      ...
      >
        ...
        <footer className="border-t border-border bg-background px-6 py-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-muted-foreground">
            <span>
              &copy; {new Date().getFullYear()} RentalMkt — Vendor Portal
            </span>
