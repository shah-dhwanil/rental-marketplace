...
import { ArrowLeft, Plus, Pencil, Trash2, Cpu, Check, X } from "lucide-react";
...
export function VendorProductDevicesPage() {
    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
        ...
        <div className="flex gap-2 shrink-0">
          <Link to={`/vendor/dashboard/products/${productId}/edit`}>
            <Button variant="outline" size="sm">
              Edit Product
            </Button>
          </Link>
          <Link to={`/vendor/dashboard/products/${productId}/devices/create`}>
      ...
      {!loading && devices.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-border rounded-xl">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Add individual device units for this product.
          </p>
          <Link to={`/vendor/dashboard/products/${productId}/devices/create`} className="mt-4">
            <Button size="sm" className="gap-1.5">
              <Plus className="size-4" /> Add Device
            </Button>
          </Link>
