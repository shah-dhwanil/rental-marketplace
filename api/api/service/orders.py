...
from api.models.orders import CreateOrderRequest, UpdateOrderStatusRequest
from api.models.orders import CreateOrderResponse, OrderResponse
...
class OrderService:
        ...
        try:
            ...
            if data.status == "completed" and data.defect_charge and caller_role == "vendor":
                try:
                    ...
                    from api.repository.defects import DefectRepository
                    from api.service.defects import DefectService
                    from api.models.defects import CreateDefectChargeRequest
    ...
    async def get_invoice_pdf(self, order_id: str, caller_id: str, caller_role: str) -> bytes:
        from api.service.pdf import generate_invoice_pdf
        order = await self._orders.find_by_id(UUID(order_id))
        if not order:
            raise OrderNotFoundException(order_id)
        self._assert_access(order, caller_id, caller_role)
    ...
    async def get_contract_pdf(self, order_id: str, caller_id: str, caller_role: str) -> bytes:
        from api.service.pdf import generate_contract_pdf
        order = await self._orders.find_by_id(UUID(order_id))
        if not order:
            raise OrderNotFoundException(order_id)
        self._assert_access(order, caller_id, caller_role)
